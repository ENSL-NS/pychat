...
# Initialize the interface. This clears the screen and sets up colorama.
# The passed parameter specifies the username of the local user.
iface = interface.Interface("Francesco")

# `add_message` adds a line with a new message. The first parameter specifies
# the username of who wrote the message. The second parameter is the text to
# display. If the username corresponds to the local user, the message is
# shown with a check mark next to it.
...
# Older messages can be updated too, not only the last one.
i3 = iface.add_message("Francesco", "Did you get my first message?")
iface.add_message("Saloua", "Yes")
time.sleep(1)
iface.update_message(i3, "read")
